...
require(['masa', 'echarts'], function (Masa, echarts) {
    ...
    var dealfeatureView = Masa.View({
        ...
        initChart:function(){
            ...
            Masa.getRequest("retvisitorcount").done(function(d){
            	$.each(d, function(i, n){
        ...
        {
            Masa.getRequest("retvisitdashboard").done(function (d) {
            	var last_month = d.last_month;
            	var last_week = d.last_week;
            	var total = d.total;
            	var yesterday = d.yesterday;
            	$("#yesterday number").html(yesterday);
            	$("#yesterday i").attr("data-original-title","前天累计回访客户: "+yesterday+"<br/>环比呈稳定趋势");
            	$("#last_week number").html(last_week);
            	$("#last_week i").attr("data-original-title","大上周累计回访客户: "+last_week+"<br/>环比呈稳定趋势。")
            	$("#last_month number").html(last_month);
            	$("#last_month i").attr("data-original-title","大上个月累计回访客户: "+last_month+"<br/>环比呈稳定趋势。")
            	$("#total").html(total);
